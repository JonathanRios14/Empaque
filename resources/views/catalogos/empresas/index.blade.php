                        ...
                        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                            <div>
                                ...
                                <p class="theme-text text-sm text-gray-500">
                                    Aquí puedes ver las empresas relacionadas a marcas y productos.
                                </p>
                            </div>

                            <form method="GET" action="{{ route('catalogos.empresas.index') }}" class="catalogo-ajax-filter-form flex gap-2">
                                <div class="relative">
                                    <input type="text"
                                           name="buscar"
                                ...
                                <tr>
                                    ...
                                    <th class="px-6 py-4 text-left font-semibold">
                                        <a href="{{ $sortLink('marcas_count') }}" class="inline-flex items-center gap-2 hover:text-[#5b3a1e]">
                                            Marcas
                                            <span class="text-xs">{{ $sortIcon('marcas_count') }}</span>
                                        </a>
                                    </th>
...
<th class="px-6 py-4 text-right font-semibold">
    ...
    <a href="{{ route('catalogos.productos.index', ['empresa_id' => $empresa->id]) }}"
       class="theme-button-secondary inline-flex items-center gap-1 px-3 py-1.5 rounded-lg bg-[#f3efe7] text-[#5b3a1e] text-xs font-semibold border border-[#e5d8c7] hover:bg-[#e5d8c7] transition">
        ...
        <span>→</span>
                    ...
                    <div class="theme-soft px-6 py-3 border-t border-[#e5d8c7] theme-border bg-[#fbf8f3] flex flex-col md:flex-row md:items-center md:justify-between gap-3">
                        <p class="theme-text text-sm text-gray-500">
                            Mostrando
                            <span class="theme-title font-semibold text-[#3b2818]">{{ $empresas->firstItem() ?? 0 }}</span>
                            a
                            <span class="theme-title font-semibold text-[#3b2818]">{{ $empresas->lastItem() ?? 0 }}</span>
                            de
                            <span class="theme-title font-semibold text-[#3b2818]">{{ $empresas->total() }}</span>
                            empresa(s)
                        </p>
