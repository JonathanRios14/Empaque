...
<head>
    <meta charset="UTF-8">
    <title>Empresas | Sistema de Empaque</title>
...
<div x-data="{ sidebarOpen: true, catalogos: true, seguridad: false, produccion: false }" class="flex min-h-screen">
    ...
    <main class="flex-1">
        @include('layouts.topbar', [
            'title' => 'Empresas',
            'description' => 'Catálogo de empresas sincronizadas desde la API.'
        ])
                ...
                <div class="bg-white rounded-2xl border border-[#e5d8c7] shadow-sm overflow-hidden">
                    ...
                    <div class="p-6 border-b border-[#e5d8c7]">
                        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                            <div>
                                <h2 class="text-lg font-bold text-[#3b2818]">Listado de empresas</h2>
                                <p class="text-sm text-gray-500">Aquí puedes ver las empresas relacionadas a marcas y productos.</p>
                            </div>

                            <form method="GET" action="{{ route('catalogos.empresas.index') }}" class="flex gap-2">
                                <div class="relative">
                                    <input type="text"
                                           name="buscar"
                                ...
                                <tr>
                                    ...
                                    <th class="px-6 py-4 text-left font-semibold">
                                        <a href="{{ $sortLink('nombre') }}" class="inline-flex items-center gap-2 hover:text-[#5b3a1e]">
                                            Empresa <span class="text-xs">{{ $sortIcon('nombre') }}</span>
                                        </a>
                                    </th>

                                    <th class="px-6 py-4 text-left font-semibold">
                                        <a href="{{ $sortLink('marcas_count') }}" class="inline-flex items-center gap-2 hover:text-[#5b3a1e]">
                                            Marcas <span class="text-xs">{{ $sortIcon('marcas_count') }}</span>
                                        </a>
                                    </th>
                            ...
                            <tbody>
                                @forelse ($empresas as $empresa)
                                    <tr class="border-b border-gray-100 hover:bg-[#faf7f2] transition">
                                        <td class="px-6 py-4 text-gray-500">#{{ $empresa->id }}</td>

                                        <td class="px-6 py-4">
                                            <span class="font-semibold text-[#3b2818]">{{ $empresa->nombre }}</span>
                                        </td>

                                        <td class="px-6 py-4">
                                            <span class="px-3 py-1 rounded-full bg-[#f3efe7] text-[#5b3a1e] text-xs font-semibold border border-[#e5d8c7]">
                                                {{ $empresa->marcas_count }} marca(s)
                                            </span>
                                        </td>

                                        <td class="px-6 py-4">
                                            <span class="px-3 py-1 rounded-full bg-[#f3efe7] text-[#5b3a1e] text-xs font-semibold border border-[#e5d8c7]">
                                                {{ $empresa->productos_count }} producto(s)
                                            </span>
                                        </td>
                                    </tr>
                                @empty
                    ...
                    <div class="px-6 py-3 border-t border-[#e5d8c7] bg-[#fbf8f3] flex flex-col md:flex-row md:items-center md:justify-between gap-3">
                        <p class="text-sm text-gray-500">
                            Mostrando
                            <span class="font-semibold text-[#3b2818]">{{ $empresas->firstItem() ?? 0 }}</span>
                            a
                            <span class="font-semibold text-[#3b2818]">{{ $empresas->lastItem() ?? 0 }}</span>
                            de
                            <span class="font-semibold text-[#3b2818]">{{ $empresas->total() }}</span>
                            empresa(s)
                        </p>

                        <div>
                            {{ $empresas->onEachSide(1)->links('pagination.cafe') }}
                        </div>
                    </div>
