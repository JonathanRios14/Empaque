...
<head>
    <meta charset="UTF-8">
    <title>Capas | Sistema de Empaque</title>
...
<div x-data="{ sidebarOpen: true, catalogos: true, seguridad: false, produccion: false }" class="flex min-h-screen">
    ...
    <main class="flex-1">
        @include('layouts.topbar', [
            'title' => 'Capas',
            'description' => 'Catálogo de capas sincronizadas desde la API.'
        ])
                ...
                <div class="bg-white rounded-2xl border border-[#e5d8c7] shadow-sm overflow-hidden">
                    ...
                    <div class="p-6 border-b border-[#e5d8c7]">
                        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                            <div>
                                <h2 class="text-lg font-bold text-[#3b2818]">Listado de capas</h2>
                                <p class="text-sm text-gray-500">Aquí puedes ver las capas registradas en el catálogo.</p>
                            </div>

                            <form method="GET" action="{{ route('catalogos.capas.index') }}" class="flex gap-2">
                                <div class="relative">
                                    <input type="text"
                                           name="buscar"
                ...
                <span class="text-xs">{{ $sortIcon('productos_count') }}</span>
                            ...
                            <tbody>
                                @forelse ($capas as $capa)
                                    <tr class="border-b border-gray-100 hover:bg-[#faf7f2] transition">
                                        <td class="px-6 py-4 text-gray-500">#{{ $capa->id }}</td>

                                        <td class="px-6 py-4">
                                            <span class="font-semibold text-[#3b2818]">{{ $capa->nombre }}</span>
                                        </td>

                                        <td class="px-6 py-4">
                                            <span class="px-3 py-1 rounded-full bg-[#f3efe7] text-[#5b3a1e] text-xs font-semibold border border-[#e5d8c7]">
                                                {{ $capa->productos_count }} producto(s)
                                            </span>
                                        </td>
                                    </tr>
                                @empty
                    ...
                    <div class="px-6 py-3 border-t border-[#e5d8c7] bg-[#fbf8f3] flex flex-col md:flex-row md:items-center md:justify-between gap-3">
                        <p class="text-sm text-gray-500">
                            Mostrando
                            <span class="font-semibold text-[#3b2818]">{{ $capas->firstItem() ?? 0 }}</span>
                            a
                            <span class="font-semibold text-[#3b2818]">{{ $capas->lastItem() ?? 0 }}</span>
                            de
                            <span class="font-semibold text-[#3b2818]">{{ $capas->total() }}</span>
                            capa(s)
                        </p>

                        <div>
                            {{ $capas->onEachSide(1)->links('pagination.cafe') }}
                        </div>
                    </div>
