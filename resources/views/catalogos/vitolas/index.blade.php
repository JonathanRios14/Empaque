                ...
                <div class="bg-white rounded-2xl border border-[#e5d8c7] shadow-sm overflow-hidden">
                    ...
                    <div class="p-6 border-b border-[#e5d8c7]">
                        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                            <div>
                                <h2 class="text-lg font-bold text-[#3b2818]">Listado de vitolas</h2>
                                <p class="text-sm text-gray-500">Aquí puedes ver las vitolas registradas en el catálogo.</p>
                            </div>

                            <form method="GET" action="{{ route('catalogos.vitolas.index') }}" class="flex gap-2">
                                <div class="relative">
                                    <input type="text"
                                           name="buscar"
                ...
                <span class="text-xs">{{ $sortIcon('productos_count') }}</span>
                            ...
                            <tbody>
                                @forelse ($vitolas as $vitola)
                                    <tr class="border-b border-gray-100 hover:bg-[#faf7f2] transition">
                                        <td class="px-6 py-4 text-gray-500">#{{ $vitola->id }}</td>

                                        <td class="px-6 py-4">
                                            <span class="font-semibold text-[#3b2818]">{{ $vitola->nombre }}</span>
                                        </td>
                    ...
                    <div class="px-6 py-3 border-t border-[#e5d8c7] bg-[#fbf8f3] flex flex-col md:flex-row md:items-center md:justify-between gap-3">
                        <p class="text-sm text-gray-500">
                            Mostrando
                            <span class="font-semibold text-[#3b2818]">{{ $vitolas->firstItem() ?? 0 }}</span>
                            a
                            <span class="font-semibold text-[#3b2818]">{{ $vitolas->lastItem() ?? 0 }}</span>
                            de
                            <span class="font-semibold text-[#3b2818]">{{ $vitolas->total() }}</span>
                            vitola(s)
                        </p>
