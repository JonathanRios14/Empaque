...
<div x-data="{ sidebarOpen: true, catalogos: false, seguridad: true, produccion: false }" class="flex min-h-screen">
...
<div class="px-6 py-4 border-t border-[#e5d8c7] bg-[#fbf8f3] flex flex-col md:flex-row md:items-center md:justify-between gap-4">
    <p class="text-sm text-gray-500">
        Mostrando
        <span class="font-semibold text-[#3b2818]">{{ $usuarios->firstItem() ?? 0 }}</span>
        a
        <span class="font-semibold text-[#3b2818]">{{ $usuarios->lastItem() ?? 0 }}</span>
        de
        <span class="font-semibold text-[#3b2818]">{{ $usuarios->total() }}</span>
        usuario(s)
    </p>
